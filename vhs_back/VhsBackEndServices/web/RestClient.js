...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.VhsUserCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-vhsuser-list-itemis template identifier for item
                        templateName: '#tpl-vhsuser-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.vhsuser = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.vhsuser,
                    // tpl-vhsuser-details is template identifier for chosen model element
                    templateName: '#tpl-vhsuser-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.VhsUser();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-vhsuser-details is a template identifier for chosen model element
                templateName: '#tpl-vhsuser-details',
