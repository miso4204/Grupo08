...
// Model for VhsSpecialOffer entity
    models.VhsSpecialOffer = Backbone.Model.extend({
        urlRoot: "http://localhost:8087/VhsBackEndServices/webresources/vhsspecialoffer/",
        idAttribute: 'idSpecialOffers',
        defaults: {
            price: "",
            description: "",
            offerCity: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idSpecialOffers');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8087/VhsBackEndServices/webresources/vhsspecialoffer/';
            }
    ...
    // Collection class for VhsSpecialOffer entities
    models.VhsSpecialOfferCollection = Backbone.Collection.extend({
        model: models.VhsSpecialOffer,
        url: "http://localhost:8087/VhsBackEndServices/webresources/vhsspecialoffer/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.VhsSpecialOfferCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-vhsspecialoffer-list-itemis template identifier for item
                        templateName: '#tpl-vhsspecialoffer-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.vhsspecialoffer = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.vhsspecialoffer,
                    // tpl-vhsspecialoffer-details is template identifier for chosen model element
                    templateName: '#tpl-vhsspecialoffer-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.VhsSpecialOffer();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-vhsspecialoffer-details is a template identifier for chosen model element
                templateName: '#tpl-vhsspecialoffer-details',
        ...
        getData: function () {
            return {
                idSpecialOffers: $('#idSpecialOffers').val(),
                price: $('#price').val(),
                description: $('#description').val(),
                offerCity: $('#offerCity').val(),
                longitude: $('#longitude').val(),
                mainImageUrl: $('#mainImageUrl').val(),
